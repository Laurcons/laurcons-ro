...
import A from '@/components/ui/old/A';
  ...
  return (
    <>
      <Alert className="mb-10">
        <AlertDescription>
          You are visiting a page from the old version of my website, provided for your convenience.
          <Button className="my-2" size="sm" asChild>
            <a href="/">Visit my new landing page</a>
          </Button>
        </AlertDescription>
      </Alert>
      ...
      <div className="overflow-hidden">
        {project.imageUrl && (
          <div className="md:float-right mx-auto md:ml-4 mb-4">
            <img className="max-w-xs mx-auto border rounded-md" src={project.imageUrl} />
          </div>
        )}
        <Markdown className="markdown">{project.shortDescMarkdown}</Markdown>
        <hr className="mt-5 mb-1" />
        <div className="text-gray-500 mb-4">Published: {project.creationDate.format('YYYY-MM-DD')}</div>
        <div className={'overflow-hidden border rounded-md mb-4 ' + (project.imageUrl ? '' : 'md:float-right md:ml-4')}>
          <div className="px-3 py-2 border-b rounded-t-md bg-sky-800">Versions and Links</div>
          {project.versions.map((version, idx) => (
            <div className={'px-3 py-2' + (idx + 1 !== project.versions.length ? ' border-b' : '')}>
              {version.name}:{' '}
              {version.links
                .map((link) => (
              ...
              {version.releaseDate && (
                <span className="text-gray-500 ml-1">({version.releaseDate.format('YYYY-MM-DD')})</span>
              )}
            </div>
          ))}
        </div>
        {project.oldSiteLink && (
          <div className="inline-block text-white border rounded bg-red-900 px-3 py-2 mb-3">
            This is a {dayjs().get('year') - project.creationDate.get('year') + 1}
